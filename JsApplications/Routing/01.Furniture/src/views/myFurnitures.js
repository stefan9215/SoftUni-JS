import {html, until} from "../lib.js";
import {getMyFurniture} from "../api/data.js";

const myFurnitureTemplate = (furniturePromise) => html`
    <div class="row space-top">
        <div class="col-md-12">
            <h1>My Furniture</h1>
            <p>This is a list of your publications.</p>
        </div>
    </div>
    <div class="row space-top">
        ${until(furniturePromise, html`<p>Loading Furniture &hellip;</p>`)}
    </div>`;

const furnitureTemplate = (furniture) => html`
    <div class="col-md-4">
        <div class="card text-white bg-primary">
            <div class="card-body">
                <img src="${furniture.img}"/>
                <p>${furniture.description}</p>
                <footer>
                    <p>Price: <span>${furniture.price} $</span></p>
                </footer>
                <div>
                    <a href=${`/details/${furniture._id}`} class="btn btn-info">Details</a>
                </div>
            </div>
        </div>
    </div>`;

export function myFurniturePage(ctx) {
    const id = JSON.parse(sessionStorage.getItem('userData')).id;
    ctx.render(myFurnitureTemplate(getMyItems(id)));
}

async function getMyItems(id) {
    const result = await getMyFurniture(id);

    return result.map(furnitureTemplate);
}